module Enterprise::Account::PlanUsageAndLimits # rubocop:disable Metrics/ModuleLength
  ...
  def default_captain_limits
    ...
    # If the plan name is not present or not in the quota map, allow max usage
    return max_limits if plan_name.blank?

    begin
      plan_quota = JSON.parse(plan_quota) if plan_quota.is_a?(String)
      plan_quota[plan_name.downcase] || max_limits
    rescue StandardError
